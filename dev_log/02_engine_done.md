...
## Amendment (during Unit 04 review): zero-transaction stops must still cost door-dwell time

While validating Unit 04's implementation, its `fairness.test.ts` development surfaced a third instance of the same infinite-loop bug class (see the two amendments above): a **full elevator** (`capacityRemaining === 0`) told to `'stop'` at a floor with only an active pickup call — nothing onboard destined there, and no capacity to board anyone — boards nobody and alights nobody. `computeDoorDwellMs` returned `0` for zero passengers, so the doors-finished-dwell event re-fired the identical decision at the identical timestamp forever. This is not a rare edge case: with realistic random-generated passenger load (which is exactly what Unit 04 exists to produce), the Unit 04 agent found this reproduced with `fcfsNearestCar` and `scanLook` across roughly 20% of random seeds at moderate load, and 100% of seeds tried at higher load.

Root cause was different from (though the same *class* as) the boarding-direction bug above: `computeDoorDwellMs`'s only real caller (`handleStop`) is *only ever* invoked once the doors have already physically opened — a stop with zero passengers is not "nothing happened," it's still a real door-open/close cycle that takes time, just with no one using it. Returning `0` for that case was the actual bug, not a downstream symptom. Fixed at the source: `computeDoorDwellMs` now always costs at least `base` (only the *additional*-passenger increment is conditional on `total > 1`), so a zero-transaction stop can never produce a zero-ms dwell regardless of *why* nobody boarded or alighted — capacity exhaustion, a direction mismatch, or any other future cause. This is a strictly more robust fix than patching each specific cause (e.g. adding a capacity check to every algorithm's `'stop'` logic) since it closes the entire bug class at the one place all such stops funnel through.

Added a direct regression test to `capacity.test.ts` (a full elevator repeatedly told to stop for a pickup it can't serve, asserting the simulation terminates and every door-open/close pair has non-zero duration) and updated `door.test.ts`'s zero/negative-passenger-count expectations from `0` to `base`. One pre-existing Unit 02 test (`simulation.test.ts`'s mid-route redirection test) had a hand-computed timestamp shift by `+3000ms` (one `doorDwellBaseMs`) for a stop where nobody boards — corrected, not a new bug. 80/80 tests pass across the whole project; lint/format/build re-verified clean.

**Residual risk — resolved same-day, per developer instruction**: a `BuildingConfig.doorDwellBaseMs` of exactly `0` would still have reproduced a zero-ms dwell and the same infinite loop. Rather than waiting for Unit 06's UI to be the first thing constructing a `BuildingConfig` from untrusted input, `runSimulation` now validates `doorDwellBaseMs > 0` (via `!(config.doorDwellBaseMs > 0)`, which also rejects `NaN`) up front and throws a descriptive error — the single place every `BuildingConfig` funnels through, regardless of caller. Two regression tests added to `simulation.test.ts` (rejects zero/negative, accepts positive). 82/82 tests pass.
