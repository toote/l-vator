...
**Known residual risk, explicitly not fixed here**: a `BuildingConfig.doorDwellBaseMs` of exactly `0` would still reproduce a zero-ms dwell and the same infinite loop, since `base` itself would be `0`. Nothing in the codebase currently constructs a `BuildingConfig` with `doorDwellBaseMs: 0` (all defaults and Unit 04's example scenario use realistic positive values), so this is a latent, currently-unreachable risk rather than an active bug — worth validating (e.g. `doorDwellBaseMs > 0`) once a unit constructs `BuildingConfig` from untrusted or UI input, most likely Unit 06.
