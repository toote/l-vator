...
## Amendment (post-Unit-10, at developer request): a fourth arrival pattern, `lunch-peak`

Developer-requested: "a mix between peaks, each floor has its own rate of arrival. floor 0 can have any destination but any passenger on other floors will have 0 as destination floor." Added `'lunch-peak'` to `ArrivalPattern` — named after the standard elevator-traffic term for exactly this shape (simultaneous bidirectional lobby traffic, as opposed to the one-directional `up-peak`/`down-peak`).

Generates from every floor, like `random` (not just floor 0, like `up-peak`) — so the existing per-floor-rate-override mechanism (`RandomArrivalParams.floorRates`) already applies to it with no new mechanism needed, matching "each floor has its own rate of arrival." Destination selection is origin-dependent, not purely pattern-dependent (a first for this file): floor 0 follows `up-peak`'s rule (uniform random 1..floorCount); every other floor follows `down-peak`'s rule (always 0). Factored the destination-selection ternary chain out into a small `destinationFor` helper now that a 4th, origin-dependent branch exists — `up-peak`'s and `down-peak`'s own logic (and RNG draw sequence, preserving byte-for-byte reproducibility for existing patterns/seeds) is unchanged, just relocated.

`generatingFloors` (already exported for the UI's per-floor-rate panel to reuse) returns all floors for `lunch-peak`, same as `random`. `configPanel.ts`'s pattern dropdown gained the new option; `validation.ts`'s existing `floorCount >= 1` guard (originally added for `up-peak`'s `uniformFloor(1, floorCount)` call) already covers `lunch-peak` too, since it reuses that same call for floor 0's destinations — no new validation needed.

Five new tests in `randomArrivals.test.ts`: the two destination rules (floor 0 vs. other floors) each proven directly, confirmation it generates from every floor (not just floor 0), and a dedicated proof that a `floorRates` override on a non-zero floor actually takes effect (unlike under `up-peak`, where it would be a silent no-op) — directly validating the developer's own "each floor has its own rate" framing. 230/230 tests pass; lint/build re-verified clean. Verified live: selected in the config panel, per-floor-rate panel correctly showed every floor as overridable, and a full batch run produced real, distinct per-algorithm results — including a genuine, interesting finding that SCAN/LOOK handles this bidirectional traffic far worse than the assignment-based algorithms (~90% unserved vs. ~5% in one verification run), exactly the kind of algorithm-comparison insight this project exists to surface.
