...
## Amendment (post-Unit-10, at developer request): a description for each algorithm

Developer-requested: "add a good description of each of the algorithms" — the checkbox list only ever showed a bare display name, with no explanation of what each strategy actually does, working against this project's own stated teaching-first objective (`00_main.md`'s "What This Is": "should be usable by someone other than the author to build intuition about dispatch algorithms").

`Algorithm` (`src/algorithms/types.ts`) gained a required `description: string` field — one or two plain-language sentences describing each algorithm's *current, real* dispatch behavior (not an idealized textbook description), written after re-reading each algorithm's own file to confirm accuracy against its actual logic as it stands today, post every fix this project has accumulated (e.g. FCFS's description reflects that it now respects direction for cars actually carrying passengers, only staying "naive" for empty ones — see `03_algorithms_done.md`'s most recent amendment). Populated for all six algorithms, including the three homing variants, whose description is phrased as "[base strategy], plus one addition" rather than repeated from scratch.

`algorithmSelect.ts` renders each description directly beneath its checkbox, always visible (not a hover-only tooltip, which would work against the same teaching-first goal a missing description did) — de-emphasized via reduced opacity on the inherited text color rather than a hardcoded gray, so it stays legible in dark mode without a separate style branch (verified live in both modes).

Adding a required field to `Algorithm` surfaced a few test-only mock algorithm objects (`trialRunner.test.ts`, `computeMetrics.test.ts`, `waitingCounts.test.ts`) that structurally satisfied the OLD shape but not the new one — each is a hand-built stand-in for a real algorithm, not one of the six real ones, so each just got `description: id` (a placeholder, since testing an algorithm's own description text was never these files' concern). `algorithms.test.ts`'s existing "every discovered entry has a real Algorithm shape" test extended to assert the new field is a non-empty string on all six real algorithms. 231/231 tests pass; lint/build re-verified clean.
