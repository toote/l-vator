...
## Amendment (after the first real GitHub Pages deploy, developer-reported): base path case mismatch

Developer-reported: after the Pages workflow ran and deployed for the first time, the site loaded but its JS bundle 404'd. Root cause: this unit's own "Key Decisions" (above) recorded the repo as `L-vator`, and `vite.config.ts`'s `base: '/L-vator/'` was set to match — but the actual GitHub repo (`git remote -v`) is `toote/l-vator`, lowercase. GitHub Pages serves a project site at a path matching the repo name exactly, and that path is case-sensitive on the actual CDN even though `github.io` hostnames themselves aren't — so `index.html`'s asset references to `/L-vator/assets/...` 404'd against a site actually rooted at `/l-vator/`. Fixed by changing `base` to `/l-vator/`. `dev_log/00_main.md`'s "Completed Features" line (which also named the base path) corrected to match. No other file hardcoded the old casing outside of frozen historical dev_log records, which are left as-is per this project's convention.
