...
import { algorithm as baseAlgorithm } from './etaDispatch';
import { algorithm } from './etaDispatchHoming';
...
describe('etaDispatchHoming (hook-level)', () => {
...
describe('etaDispatchHoming (integration: proactive repositioning reduces wait time)', () => {
  function scenario() {
...
describe('algorithms discovery shape', () => {
  it('exposes a valid Algorithm entry', () => {
    expect(algorithm.id).toBe('eta-dispatch-homing');
