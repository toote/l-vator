// Zoning dispatch (strict variant), with a "returns to the lobby after sitting idle too long"
// homing policy layered on top. See dev_log/10_homing_algorithms.md for the homing mechanism's
// full rationale and dev_log/11_algorithm_expansion.md for zoning's own model.
//
// Everything except the idle fallback mirrors zoning.ts (deliberately duplicated, not shared).
// The base zoning.ts itself is completely untouched by this file.
...
/**
 * Identical to zoning.ts's refreshAssignments. Homing plays no role here: a homing elevator
 * (idle-but-drifting toward floor 0) is a completely ordinary zone-eligible candidate for its own
 * zone (floor 0 is always eligible for everyone regardless), picked up by this same logic exactly
 * like any other idle elevator -- no special-casing needed. See nearestCarDirectionalHoming.ts's
...
export const algorithm: Algorithm = {
  id: 'zoning-homing',
  name: 'Zoning (Returns to Lobby)',
  description:
    'The Zoning strategy, plus one addition: an elevator that sits idle longer than the ' +
